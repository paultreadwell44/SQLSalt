...
Script Description: 
	this is the base CREATE TABLE code to generate the 
	reference/lookup table including wait stats.  This data 
	is taken directly from BOL [see reference]
...
('FS_FC_RWLOCK', 'Occurs when there is a wait by the FILESTREAM garbage collector to do either of the following:  Disable garbage collection (used by backup and restore), or Execute one cycle of the FILESTREAM garbage collector.'), 
...
('FS_LOGTRUNC_RWLOCK', 'Occurs when there is a wait to acquire access to FILESTREAM log truncation to do either of the following:  Temporarily disable FILESTREAM log (FSLOG) truncation (used by backup and restore), or Execute one cycle of FSLOG truncation.'), 
...
('FT_METADATA_MUTEX', 'Identified for informational purposes only. Not supported. Future compatibility is not guaranteed.'), 
